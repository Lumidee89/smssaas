...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-[#06322C]">Class Management</h1>
            <p class="text-slate-500 mt-1">Manage all classes in your school</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 border border-slate-100">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-slate-400 text-sm">Avg Class Size</p>
                    <h3 class="text-2xl font-bold text-[#06322C]">{{ $averageClassSize }}</h3>
                </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm p-4 mb-6">
        <div class="grid md:grid-cols-2 gap-4">
            <div>
                <input type="text" id="search" placeholder="Search by class name or section..." 
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-slate-50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Class Name</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Section</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Class Teacher</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Students</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Subjects</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Capacity</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-slate-50 transition">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="w-10 h-10 rounded-full bg-emerald-50 flex items-center justify-center mr-3">
                                    <svg class="w-5 h-5 text-[#3A7B72]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                                    </svg>
                                </div>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-wrap gap-1">
                                @forelse($class->subjects->take(3) as $subject)
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                            @if($class->capacity)
                                ...
                                @php
                                    $percentage = ($class->students_count / $class->capacity) * 100;
                                @endphp
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center text-slate-400">
                            ...
                            <p class="text-sm mt-1">Click "Add New Class" to get started</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
...
<script>
    ...
    function filterTable() {
        ...
        rows.forEach(row => {
            ...
            if (searchTerm && !className.includes(searchTerm) && !section.includes(searchTerm) && !teacher.includes(searchTerm)) {
                show = false;
            }
