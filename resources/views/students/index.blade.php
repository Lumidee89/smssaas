...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-[#06322C]">Student Management</h1>
            <p class="text-slate-500 mt-1">Manage all students in your school</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 border border-slate-100">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-slate-400 text-sm">Male Students</p>
                    <h3 class="text-2xl font-bold text-[#06322C]">{{ $maleStudents }}</h3>
                </div>
                <div class="w-12 h-12 bg-emerald-50 rounded-full flex items-center justify-center">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 border border-slate-100">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-slate-400 text-sm">Female Students</p>
                    <h3 class="text-2xl font-bold text-[#06322C]">{{ $femaleStudents }}</h3>
                </div>
                <div class="w-12 h-12 bg-emerald-50 rounded-full flex items-center justify-center">
    ...
    <div class="bg-white rounded-2xl shadow-sm p-4 mb-6">
        <div class="grid md:grid-cols-4 gap-4">
            <div>
                <input type="text" id="search" placeholder="Search by name or admission number..." 
                       class="w-full px-4 py-2 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-[#3A7B72]">
            </div>
            <div>
                <select id="class_filter" class="w-full px-4 py-2 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-[#3A7B72]">
                    <option value="">All Classes</option>
                    @foreach($students->pluck('class.name')->unique() as $className)
                        <option value="{{ $className }}">{{ $className }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <select id="gender_filter" class="w-full px-4 py-2 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-[#3A7B72]">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-slate-50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Admission No</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Student Name</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Class</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Gender</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-slate-50 transition">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="w-8 h-8 rounded-full bg-emerald-50 flex items-center justify-center mr-3">
                                    <span class="text-[#3A7B72] font-semibold text-sm">{{ substr($student->first_name, 0, 1) }}{{ substr($student->last_name, 0, 1) }}</span>
                                </div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500 capitalize">{{ $student->gender }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{{ $student->email }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{{ $student->phone ?? 'N/A' }}</td>
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center text-slate-400">
                            ...
                            <p class="text-sm mt-1">Click "Add New Student" to get started</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
...
<script>
    ...
    document.getElementById('gender_filter').addEventListener('change', function() {
        filterTable();
    });
    ...
    function filterTable() {
        ...
        const genderFilter = document.getElementById('gender_filter').value.toLowerCase();
        
        const rows = document.querySelectorAll('tbody tr');
        
        rows.forEach(row => {
            const admissionNumber = row.cells[0]?.textContent.toLowerCase() || '';
            ...
            if (searchTerm && !admissionNumber.includes(searchTerm) && !studentName.includes(searchTerm)) {
                show = false;
            }
            
            if (classFilter && !studentClass.includes(classFilter)) {
                show = false;
            }
